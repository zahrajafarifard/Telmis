...
const Category = () => {
  ...
  useEffect(() => {
    const _cat = async () => {
      ...
      _response = await fetch(
        `${process.env.REACT_APP_URL}/exchange/getFileType/${categoryId}`,
        {
          method: "GET",
          headers: {
  ...
  const deleteCategoryHandler = async () => {
    let _response, _data;
    _response = await fetch(
      `${process.env.REACT_APP_URL}/exchange/delete-fileType`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
    ...
    switch (_response.status) {
      ...
      case 409:
        setResponse({ code: 409, msg: "شماقادر به حذف این فرمت فایل نیستید" });
        break;
  ...
  const addCategoryHandler = async () => {
    ...
    _response = await fetch(
      `${process.env.REACT_APP_URL}/exchange/addFileType`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const editCategoryHandler = async () => {
    ...
    _response = await fetch(
      `${process.env.REACT_APP_URL}/exchange/editFileType`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({
          fileTypeId: editingCategory?.id,
          fileTypeName: editingCategory?.type,
        }),
      }
    );
  ...
  return (
    <div>
      {response.code !== 0 && (
        <div className="fixed top-[205px] right-[20%] w-fit whitespace-nowrap text-[#434242] text-sm flex flex-row bg-[#FFF099] py-2 pr-14 pl-24  rounded-e-sm my-auto">
