...
import ExchangeRegistrationDetails from "./exchange-registration-details";
  ...
  useEffect(() => {
    const _notifs = async () => {
      ...
      _response = await fetch(
        `${process.env.REACT_APP_URL}/exchange/notification`,
        {
          method: "POST",
          headers: {
    ...
    socket.on("connect", () => {
      socket.on("exchangeRegisteration", (notif) => {
      ...
      {notifications?.length ? (
        <div>
          {notifications.some(
            ...
            <div className="bg-[#B71F2633] bg-opacity-[20%] w-fit my-10  text-[#1E1B1B] text-sm px-4 py-0.5 rounded-[4px] ">
              جدید ها
            </div>
          )}
          {notifications?.map(
          ...
          {notifications.some(
            ...
            <div className="text-[#221D23] text-sm bg-[#D9D9D9]   w-fit my-10 px-4 py-0.5 rounded-[4px] ">
              مشاهده شده
            </div>
          )}
          {notifications?.map(
