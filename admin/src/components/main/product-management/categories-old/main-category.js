...
const Category = ({ setReloading, reloading, categoryId, setCategoryId }) => {
  ...
  useEffect(() => {
    const _category = async () => {
      ...
      _response = await fetch(
        `${process.env.REACT_APP_URL}/categories/getCategory/${categoryId}`,
        {
          method: "GET",
          headers: {
  ...
  const editCategoryHandler = async () => {
    ...
    _response = await fetch(
      `${process.env.REACT_APP_URL}/categories/${editingCategory.id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        ...
        <div className="rounded-b-[8px] border border-t-0 py-4 px-10  ">
          <div className="flex flex-row ">
            ...
            <div
              ...
              className="flex mb-4 cursor-pointer mx-6"
            >
          ...
          {showCategory === "add" && (
            <div className="flex flex-row justify-between">
              <input
                ...
                className="bg-[#F7F7F7] text-[#8F8F8] p-4 text-xs w-[95%] rounded-[5px] focus:outline-none"
              />
              <div
